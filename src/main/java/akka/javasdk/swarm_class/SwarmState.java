...
public record SwarmState(
  ...
  public SwarmState resumed(String message) {
    return new SwarmState(
        Status.RUNNING, userMessage, currentTurn, maxTurns,
        Optional.empty(), Optional.empty(), Optional.empty(),
        result, failureReason, conversationHistory);
  }
  ...
  public <R> SwarmResult<R> toSwarmResult() {
    return switch (status) {
      ...
      case PAUSED -> new SwarmResult.Paused<>(pauseReason.orElseThrow(), currentTurn, maxTurns);
