...
public class AgentTeamEvaluatorConsumer extends Consumer { // <1>
  ...
  public Effect onStateChanged(AgentTeamState state) { // <2>
    if (state.status() == AgentTeamState.Status.COMPLETED) {
      evalToxicity(state);
  ...
  private void evalToxicity(AgentTeamState state) {
  ...
  private void evalSummarization(AgentTeamState state) {
